...
**Done quand**
- [ ] Session réelle : calibration 45 s à la webcam puis composer un mot au regard seul (cibles adaptées), démontré sur FLU-REF-4 ou équivalent. *(Moteur **et câblage hub/contrat** livrés et testés — le hub accepte le regard + la calibration de bout en bout, le chemin souris préservé ; **reste** : source regard MediaPipe du client + outil `record-gaze` + une session humaine réelle = action physique.)*
...
- **Reprise session suivante** : finir la Phase 6 (source MediaPipe + record-gaze côté client, puis session webcam réelle → `phase-6-done`) ; reste de la Phase 7 (Tauri+watchdog+installeurs signés, espace aidant web — listing + recalibration, sauvegarde/restauration avec test CI, signatures minisign, soak 72 h, install < 30 min par un tiers, runner FLU-REF) — dont plusieurs critères exigent une action physique/matérielle.
