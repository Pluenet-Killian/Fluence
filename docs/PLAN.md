...
- **Reprise session suivante** : câbler le regard de bout en bout (hub + client MediaPipe + record-gaze), puis session webcam réelle → `phase-6-done` ; reste de la Phase 7 (Tauri+watchdog+installeurs signés, espace aidant, sauvegarde/restauration avec test CI, signatures minisign, soak 72 h, install < 30 min par un tiers, runner FLU-REF) — dont plusieurs critères exigent une action physique/matérielle.
