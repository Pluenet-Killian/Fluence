...
**Done quand**
- [ ] Session réelle : calibration 45 s à la webcam puis composer un mot au regard seul (cibles adaptées), démontré sur FLU-REF-4 ou équivalent. *(**Toute la chaîne logicielle est livrée et testée** — moteur, câblage hub/contrat, **client MediaPipe + calibration + `record-gaze`** (opt-in, chemin souris préservé). **Reste** : provisionner les assets MediaPipe offline (`docs/demos/phase6-gaze.md`) + une session humaine réelle = action physique, hors automatisation.)*
- [~] Datasets de regard versionnés + chiffre de précision publié en nightly. *(**Mécanisme livré** : replay T4 `evaluate` + gate `xtask gaze-accuracy` publié en nightly ; datasets **synthétiques** versionnés (`synthetic_grid`, gate de non-régression, pas une revendication de précision réelle). Datasets **réels** = capture `record-gaze`, à venir.)*
- [ ] Qualité de calibration visible en temps réel (base de l'espace aidant). *(Métrique livrée — `Calibrator::quality`/dérive ; exposition temps réel = espace aidant, Phase 7.)*
...
- [ ] Budgets §5.A tenus en `contractual` sur machine de référence (palier réduit). *(**Action physique** : exige le runner self-hosted FLU-REF ; seuils `provisional` ×2,5 tenus en CI. Bascule documentée `docs/ops/soak.md`.)*
- [x] **KS% ≥ 25 %** (sans contexte conversationnel) sur le corpus d'éval complet. *(**ATTEINT par le système accéléré** : rephrase LLM **29,52 % KS% hors-domaine** sur le split test v1 ≥ 25 % (Phase 4 #31, ADR-0008 ; rephrase est sans contexte conversationnel). La baseline n-gram **hors-domaine** seule est à **11,71 %** (< 25 %) — context-free mais insuffisante ; le LLM la dépasse. **Correction d'un brouillon erroné** : le 35,49 % de `run-eval` est **in-domain sur la graine v0** (sur-appris, ADR-0008 §«&nbsp;gonflé&nbsp;»), **jamais** une mesure A1.)*
- [~] Soak 72 h : zéro crash, RSS borné, zéro perte de draft (kills aléatoires inclus). *(**Mécanisme + proxy livrés** : `kill_cycles_keep_rss_bounded` (RSS bornée, warm-up, `FLUENCE_SOAK_CYCLES`) + soak nightly étendu (300) + kill-test draft ≤ 1 s ; le **72 h one-shot sur FLU-REF** = action physique, protocole `docs/ops/soak.md`.)*
- [ ] Installation chronométrée < 30 min par un tiers. *(**Action physique** : guide pas-à-pas livré (`docs/install/`) + watchdog (`fluence-watchdog`) + scaffold Tauri ; reste l'**installeur signé** (gate certificats) puis le chrono par un tiers.)*
- [x] Suite complète T1–T6 verte sur les deux OS ; couvertures aux gates. *(CI verte Win+Linux ; gate couverture `fluence-protocol` ≥ 85 % + `fluence-input` ≥ 85 % (ajouté §7 close).)*
- [ ] Tag `phase-7-done` = **A1 atteint**. *(**Pas encore posé** : la chaîne logicielle A1 est complète et testée, mais **3 critères exigent une action physique** (FLU-REF `contractual`, soak 72 h, install < 30 min par un tiers + installeur signé). Jamais coché à tort — PLAN §0.8. Le tag se pose quand ces actions sont faites.)*
...
| 6 — Regard | 🟡 moteur + gate d'exactitude livrés (2026-06-14) | — (→ `phase-6-done`) | **moteur regard complet et testé** dans `fluence-input` (One Euro, I-VT fixation/saccade, fusion pondérée + « regard désigne, tête affine », magnétisme plafonné 40 %, modèle de bruit par utilisateur, **calibration ridge** par profil — collecte/fit/maj continue lissée/dérive/format versionné, `GazePipeline` assemblé) + **gate d'exactitude T4** (`evaluate` replay + `xtask gaze-accuracy` synthétique publié en nightly) + **câblage hub/contrat** (messages WS `cal.sample`/`cal.fit`, `GazePipeline` par connexion, routage souris/regard par source — chemin souris préservé, e2e vert, tests hub déterministes) + **client regard** (`apps/web-client` : estimation MediaPipe pure+testée, source webcam opt-in, calibration express, `record-gaze` → dataset réel ; chemin souris intact, e2e vert) ; **reste pour `phase-6-done`** : provisionner les assets MediaPipe offline + **session webcam réelle** (action physique) ; piste ML-regard (6.5) = post-A1 par conception |
| 7 — Durcissement → A1 | 🟡 chaîne logicielle A1 complète (2026-06-14) | — (→ `phase-7-done`) | **7.1–7.7 livrés et testés** : sauvegarde/restauration chiffrées + kit de secours (QR+phrase) + purge (#51/#55, restauration = test CI) ; minisign + GC modèles (`fluence-models`, vrai vecteur, #54) ; passe sécurité (threat model vérifiable + robustesse property-based + SECURITY exact, #53) ; espace aidant web (santé+appareils+révocation 2-clics+journal, #49/#56/#58) ; soak proxy durci + protocole 72 h (#57) ; doc install/dépannage/trackers (#59) ; watchdog hub testé + scaffold Tauri (#60) ; **fix fiabilité** : thread store joint au drop (#52, supprime un SIGABRT de teardown). **Critères A1** : **KS% ≥ 25 % hors-domaine ATTEINT** (rephrase LLM **29,52 %** sur le split test v1, #31/ADR-0008 ; n-gram hors-domaine 11,71 %) ; T1–T6 verts 2 OS + gates couverture. **Reste = actions physiques** (pas de tag) : FLU-REF `contractual`, soak 72 h one-shot, installeur signé + install < 30 min par un tiers |

*Mise à jour de ce tableau à chaque fin de session de travail ; re-détaillage du plan à chaque fin de phase.*

### Journal de session

**Session 9 — 2026-06-14 — Phase 7 « Durcissement → A1 » : chaîne logicielle complète.**
- **Audit d'abord, puis 7.1–7.7 en autonomie**, chaque brique en TDD, PR squash sur `main` protégée, CI verte Win+Linux à chaque fois.
- **Fiabilité (débloque le train de merge)** : le thread propriétaire du store était **détaché** → son checkpoint WAL final courait contre l'arrêt du process (SQLCipher/OpenSSL) = **SIGABRT intermittent** au teardown de `next_chars`. Fix : `Arc<StoreThread>` joint le thread au drop (#52). Test de régression (WAL drainé après un drop sans `close`).
- **7.3 Données** : `RecoverySecret` (256 bits → phrase hex groupée + checksum, = clé brute SQLCipher, pas de KDF) ; `back_up`/`restore` re-chiffrés via `sqlcipher_export` (réutilise le chiffrement audité) ; `purge_content` (VACUUM). **Round-trip entre deux clés machine = test CI** (#51). Commandes offline du **binaire hub** `backup`/`restore`/`wipe` + **kit imprimable** (QR `qrcode` + phrase) réutilisant `store_paths` (#55).
- **7.4 Modèles** : crate `fluence-models` (manifeste source-unique + `verify_manifest_signature` **minisign** prouvée contre le **vrai vecteur officiel** + `verify_sha256` + `plan_gc`) ; `download-test-assets` vérifie la signature fail-closed quand configurée ; `cargo xtask models-gc` (#54).
- **7.7 Sécurité** : `docs/security/threat-model.md` (checklist §9.A → mitigation+test) ; **robustesse property-based** des entrées non fiables (`fluence-input/tests/robustness.rs` : jamais de panic + finitude sur f64 adverses) ; SECURITY.md rendu exact (#53).
- **7.2 Espace aidant** : route DELETE `devices/{id}` (révocation, #49) + GET `devices` (liste, #56) ; **page web `#care`** (santé + appareils + révocation 2-clics + journal, scope `care`, jamais de P0) + 3 méthodes SDK testées (#58). *(Reste gated sur session regard physique : qualité calibration temps réel + recalibration.)*
- **7.6 Soak** : `kill_cycles_keep_rss_bounded` (baseline **après warm-up** → supprime le flake RSS, `FLUENCE_SOAK_CYCLES`) + job nightly étendu (300) + `docs/ops/soak.md` (protocole 72 h FLU-REF) (#57).
- **7.5 Doc** : `docs/install/` (guide pas-à-pas, carte « si ça ne marche plus » gros corps, matrice trackers niveaux 0/1) (#59).
- **7.1 Desktop** : crate `fluence-watchdog` (superviseur de processus pur std, testé 2 OS, redémarrage < 2 s, backoff plafonné) + scaffold Tauri (`tauri.conf.json` + README architecture/build/**gate signature**) (#60).
- **Critère A1 KS%** : la cible est **hors-domaine sur v1** (D-12.2, ADR-0008). **ATTEINT** par le rephrase LLM (**29,52 % KS% hors-domaine** sur le split test v1, #31) ; la baseline n-gram hors-domaine est **11,71 %** (< 25 %). *(L'audit final a corrigé un brouillon qui citait à tort le **35,49 % in-domain** de `run-eval` — gonflé, non-A1 par ADR-0008. `run-eval` reste l'outil ; mais le chiffre A1 est la mesure hors-domaine v1, pas l'in-domain seed.)*
- **Reste pour `phase-7-done` = A1 (actions physiques uniquement)** : runner FLU-REF (seuils `contractual`), soak 72 h one-shot, installeur **signé** (certs) + installation < 30 min chronométrée par un tiers. La chaîne logicielle A1 est complète, testée, CI verte ; le tag n'est **pas** posé tant que ces actions physiques ne sont pas faites (PLAN §0.8). Côté Phase 6 : provisionner MediaPipe + session webcam réelle → `phase-6-done`.

**Session 8 — 2026-06-14 — Phase 6 « Le regard » : moteur + gate d'exactitude.**
- **Moteur regard complet et testé** dans `fluence-input` (pur, clock-free, déterministe — rejouable), construit module par module en TDD :
  - **`one_euro`** : filtre One Euro (Casiez CHI 2012), lisse au repos / réactif en mouvement, `min_cutoff`/`beta` par profil.
  - **`fixation`** : I-VT (seuil de vitesse) → `Fixation`/`Saccade`/`Lost` ; le dwell ne progresse que sur fixation, tolérance aux micro-pertes.
  - **`fusion`** : fusion pondérée par confiance, **« regard désigne, tête affine »** (offset tête borné à la zone), **magnétisme linguistique plafonné à 40 %** de l'espacement (agentivité prouvée par property test), **modèle de bruit par utilisateur** (variance EW des commits → taille de cible effective).
  - **`calibration`** : régression **ridge** maison (solveur Gauss-Jordan sans dépendance), `Calibrator` (collecte pursuit, fit, **maj continue lissée** sur commits non corrigés, **détection de dérive** médiane > 1,5× touche sur 30 s, qualité = RMS visible), **profil versionné serde** par contexte.
  - **`gaze`** : `GazePipeline` assemblé (calibration→fusion→One Euro→I-VT→magnétisme→dwell ; saccade/perte **maintiennent** la jauge sans annuler). Raffinement honnête de l'ordre §4.C : I-VT gate sur le point brut (le lissage ne masque jamais une saccade) + seules les fixations avancent le lisseur.
  - **`replay`** : `evaluate` (replay T4 d'une session → % de cibles correctes) + `synthetic_grid` (datasets déterministes versionnés).
- **Gate d'exactitude T4** : `cargo xtask gaze-accuracy` rejoue les sessions synthétiques (100 % sur grilles propres, ≥ 0,9 sous jitter modéré) et **publie le chiffre en nightly** (`nightly.yml`, job dédié, gate à 0,95). **Honnêteté** : données **synthétiques** = gate de correction/non-régression du pipeline, **pas** une revendication de précision réelle ; la précision réelle exige une capture réelle (`record-gaze`), distincte (clause de pivot §6).
- **Reste pour `phase-6-done`** : **source regard MediaPipe** dans le composeur + outil `record-gaze` + une **session webcam réelle** (action physique, hors automatisation). Le **câblage hub/contrat est fait** (#48 : messages `cal.sample`/`cal.fit`, `GazePipeline` par connexion, routage souris/regard). Piste ML-regard (6.5) = post-A1 par conception (SPEC §4.C). **Pas de tag `phase-6-done`** tant que la session réelle n'est pas démontrée (jamais de case cochée à tort — PLAN §0.8).
- **Passe d'audit adversarial (Phase 7.7)** : revue en 4 domaines (moteur regard, sécurité/fiabilité hub, client/SDK/e2e, ML/contrats/CI) → 6 correctifs réels en TDD : code d'appairage **comparé à temps constant** ; urgence **désarmée seulement après succès** (sinon reste armée + signale l'échec) ; `Composer.close()` (nettoyage timers/socket, plus de reconnexion orpheline) ; SDK **ignore** une frame SSE mal formée ; calibration **rejette les features non finies** ; `measure.py` **refuse** une mesure sans split TEST gelé (honnêteté §0.8). Faux positifs écartés avec justification (broadcast borné 256, `TargetMap` borné par la limite de corps, fuite P0 déjà couverte par un kill-test).
- **Phase 6 — chaîne logicielle complète** : moteur + hub + **client regard** (MediaPipe + calibration + `record-gaze`) livrés et testés (les parties pures ; la glue webcam est structurée + documentée). **Reste pour `phase-6-done`** : provisionner les assets MediaPipe offline (`docs/demos/phase6-gaze.md`) + une **session webcam réelle** (humain + caméra) — action physique.
- **Phase 7.2 entamée** : **révocation d'appareil** (`DELETE /api/v1/devices/{id}`, care, idempotent + `fluencectl revoke`) — un appareil compromis est coupé ; le store le supportait déjà, exposé via contrat+hub+CLI (2 tests).
- **Reprise session suivante** : reste de la Phase 7 (Tauri+watchdog+installeurs signés, espace aidant web — listing + recalibration, sauvegarde/restauration avec test CI, signatures minisign, soak 72 h, install < 30 min par un tiers, runner FLU-REF) — dont plusieurs critères exigent une action physique/matérielle ; côté Phase 6, provisionner MediaPipe + session webcam réelle → `phase-6-done`.
...
- **Intégration via branche d'intégration unique** : la pile #32/#35/#36 (Phase 4) + #37/#38/#39/#40 (Phase 5) se recouvrait sur les **fichiers du hub** (api/mod.rs montage+routes, state.rs `Inner`/`new_with`, config.rs struct/default/env, lib.rs `new_with`, typos.toml). Conflits résolus **une seule fois** sur `integration/phase4-5` (« garder toutes les additions » ; contrats/`api.d.ts` **régénérés** via `check-contracts` plutôt que fusionnés à la main), workspace validé vert de bout en bout, puis **squash sur `main` (#41, `c61ab06`)**.
- **Validation pré-merge (tout vert)** : Rust (`build`/`clippy -D warnings`/`fmt`/tests workspace/`check-contracts`/`check-licenses`/`cargo deny`), TS (`typecheck`/`lint`/29 tests/`build`/spectral 0), ML (`ruff`/`mypy`/118 tests). Provisionnement Piper local (exe + voix `fr_FR-siwis-medium`) + smoke-test live (RTF 0,068).
- **Hygiène dépôt** : `phase-4-done` posé et poussé sur `c61ab06` (tags `phase-0-done` → `phase-4-done`) ; PRs #32–#40 closes (#38/#40 auto-closes à la suppression de leurs bases) ; 9 branches de feature + la branche d'intégration supprimées. Plus aucune PR ouverte ; branches `origin` restantes : `main` + `feat/ipc-layer` (pré-existante, laissée intacte).
- **Reste pour clore la Phase 5** (`phase-5-done`) : brancher la **suite Playwright T5 persona** dans `integration.yml` contre le hub assemblé (+ tiny-model + Piper) — scénarios « Marc compose “bonjour” au dwell et le vocalise », acceptation de suggestion, urgence double-confirmation reçue par un 2ᵉ client, reconnexion à chaud — puis **filmer la démo reproductible** de la boucle. Dette inchangée (P0-scheduler D-3.3, opus/streaming chunké Phase 7, métriques chiffrées P2, fix génération `InputClientMessage`).
- **Reprise session suivante** : scaffolder la suite Playwright T5 (`pnpm exec playwright install`, harnais lançant le hub assemblé + tiny-model + Piper), la rendre verte Win+Linux, poser `phase-5-done`, filmer la démo.
...
- **Phase 5 — toutes les briques 5.1–5.5 livrées en PRs** (intégration restante) :
  - **5.4 urgence (#37)** : `SystemEvent::Emergency { active, at }` + `POST /api/v1/system/emergency` (control, 204) + **runtime hub** (diffusion bus avant journal best-effort, D-2.6) + **SDK `client.emergency()`** ; goldens/openapi/`api.d.ts` régénérés, spectral 0, SDK 18 tests, 2 tests hub.
  - **5.1 câblage `fluence-input`→hub (#38, sur #34)** : `PUT /input/targets` (stocke la carte), topic `input` du `/ws` fait tourner un `SelectionEngine` par connexion (amorcé depuis la carte), `ClientFrame::Input` (pointeur/contacteur/patch) → estampillé en `SelectionEvent` (focus/dwell/commit) publié sur le bus. Souris = source v0. Tests : dwell→commit sur le bus, mapping, scope.
  - **5.2 worker-tts (#39)** : crate `fluence-voice` (`VoiceBackend` ; `PiperBackend` sous-processus PCM→WAV ; `SystemVoiceBackend` SAPI/espeak « une voix toujours » ; `FallbackVoice`), `POST /voice/speak` (WAV, **ADR-0009** — opus différé Phase 7) + `GET /voice/voices`. 13 tests + smoke live Piper + 3 tests handler. P0-scheduler D-3.3 = dette.
  - **5.3 composeur web + 5.5 instrumentation (#40, sur #37)** : `apps/web-client` (Vanilla TS+Vite, AGPL) — clavier dwell+clic, 3 suggestions fixes, PARLER, urgence double-confirmation + bannière + sonnerie, SSE+WS, anti-scintillement, i18n FR, contraste élevé, **WPM/éco-frappe réels affichés** ; hub sert la PWA (`ServeDir`, `FLUENCE_WEB_DIR`). 10 tests vitest + build.
- **Découverte contrat** : `InputClientMessage` (enum interne-taggé à variantes newtype) **perd le tag `k`** à la génération TS → le composeur construit la frame fil explicitement ; fix de génération = dette.
- **Dette / différés** : **intégration Phase 5** (merge de la pile #32–#40 → hub assemblé → suite **Playwright T5** + démo filmée) ; tag `phase-4-done` au merge de #32/#35/#36 ; `phase-5-done` au merge + e2e ; P0-scheduler D-3.3 ; opus/streaming chunké (Phase 7) ; stockage chiffré des métriques (P2) ; fix génération `InputClientMessage`.
- **Reprise session suivante** : intégrer (merger la pile sur `main`), brancher la suite Playwright T5 (`integration.yml`) contre le hub assemblé + tiny-model + Piper, poser les tags, filmer la démo de la boucle.
...
- **Fait** (PRs #26 backend, #27 gestion modèles, #28 supervision+kill-test, #29 cli) :
  - **Pivot ADR-0007 (amendé)** : le binding FFI `llama-cpp-2` ne compile pas sur la cible `x86_64-pc-windows-gnu` (`llama-cpp-sys-2` cherche des `.lib` MSVC, msys2/UCRT produit des `.a` MinGW → `build.rs` assert 0 lib) → le **sous-processus `llama-server`** (binaire officiel prébuilt) devient la réalisation **principale** du backend local : pur client HTTP Rust, zéro compilation C++/CMake, **crash-isolé par la frontière de processus** (D-2.6), portable Win/Linux, testable en local. Le FFI (issue #25) reste une optimisation future derrière le trait `LlmBackend` inchangé.
  - **4.2 `LlamaServerBackend`** (`fluence-inference`, ureq) : `generate` streaming SSE annulable (`CancelToken` entre frames) ; `next_chars` via `/completion` `n_predict:1`+`n_probs` → agrégation des logprobs par **premier caractère**, renormalisée. Tests mock hermétiques + smoke-test live `#[ignore]` contre le vrai serveur. `is_healthy()` (GET /health).
  - **4.3 gestion de modèles v0** : `models/test-assets.json` (manifeste URL+**sha256**+taille), `xtask download-test-assets` (reprise HTTP `Range`, vérif sha256 = contrat d'intégrité, idempotent, `--check` sans réseau, cache `.fluence-cache/models`, override `FLUENCE_MODELS_DIR`). Tiny-LLM **SmolLM2-135M** (Apache-2.0) — *mécanique seulement*. Le binaire llama-server est de l'**infra** (provisionnée CI/dev via `FLUENCE_LLAMA_SERVER_BIN`/PATH), pas un « modèle » géré (minisign : phase 7).
  - **hub (4.5 réel)** : config `FLUENCE_LLAMA_SERVER_BIN`/`_MODEL`/`_CONTEXT` ; `SupervisedLlama` (wrapper `LlamaServerBackend` + flag `ready` partagé → tant que non prêt, `Unavailable` → **dégradation n-gram automatique**, l'Arc moteur ne change jamais) ; `supervise_llama_server` (port loopback **stable** choisi une fois, spawn binaire, poll `/health` pour la mise à *ready*, restart `Backoff` au crash, event `system.degraded`, worker visible dans `/system/health`). **stdout/stderr du serveur jetés** (llama-server peut logger le prompt = P0, §9.A). **Kill-test LLM** : binaire de test `fake-llama-server` + test d'intégration pilotant le **vrai** chemin de supervision → `/suggest` streame du `model` sain, puis **dégrade en 200 (jamais 5xx)** à la mort du serveur = **critère « Done quand » #3 prouvé en CI**, hermétique (sans modèle lourd).
  - **4.5 `fluencectl suggest --mode rephrase`** (client SSE, scope control). **Pipeline réel validé localement de bout en bout** : CLI → hub → **vrai** `llama-server` supervisé + SmolLM2 → suggestion d'origine `[model]`.
- **Découvertes / déviations notables** :
  - (a) Le harnais d'éval de la Phase 3 est **prédiction de mot** (`predict(context, word_prefix) → complétions`), or **`rephrase` est phrase-niveau** (fragment bruité → phrase corrigée complète). Mesurer le critère valeur exige donc un **nouveau mode d'évaluation** (acceptation **sémantique** par embeddings, pas lexicale) — absent du v0. C'est le cœur du travail restant, pas un simple ajout de source.
  - (b) Le **tiny-LLM valide la mécanique, pas la qualité** (PLAN §1) : « 3 propositions FR correctes » (critère #1) et le « +10 pts KS% » (critère #2) exigent un **modèle capable** (Gemma E2B / Qwen…) **et** un raffinage du prompt v0 (sur 135M, le modèle parrote l'instruction).
  - (c) **Vanguard** (machine de dev) throttle le spawn rapide de processus enfants → le kill-test `fifty_kill_cycles_leave_rss_stable` (50 cycles) échoue *localement* (prouvé environnemental : échoue identiquement sur l'arbre propre) ; vert en CI. Les autres kill-tests passent.
- **RESTE pour clore la Phase 4** — critère valeur #2 (workstream ML / nightly) : (i) **mode d'éval `rephrase`** phrase-niveau + **acceptation sémantique** (embeddings locaux, p.ex. via le `/embedding` de llama-server) dans `fluence_eval` ; (ii) **source Rephrase** pilotant le **vrai** `/suggest` du hub (réutilise le prompt `fluence-accel`, pas de réimplémentation) ; (iii) **modèle capable** provisionné en **nightly self-hosted** ; (iv) mesure `rephrase` vs n-gram, **gate +10 pts KS%**, rapport publié. Tag `phase-4-done` **non posé** tant que #2 n'est pas atteint (jamais d'ajustement d'un seuil contractuel — PLAN §0). À ouvrir en issue `debt`/`value`.
- **Reprise session suivante** : ouvrir le workstream « valeur rephrase » ci-dessus (mode d'éval phrase-niveau + acceptation sémantique + modèle capable + nightly) ; il croise la piste ML-langage. La Phase 5 (boucle complète : dwell+composeur+Piper+urgence) peut démarrer en parallèle si la valeur est traitée comme piste ML.
