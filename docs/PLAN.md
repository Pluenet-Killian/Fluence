...
- [x] **KS% ≥ 25 %** (sans contexte conversationnel) sur le corpus d'éval complet. *(**ATTEINT** : `cargo xtask run-eval` → n-gram **35,49 %** sur le corpus complet (context-free), ≥ 25 % ; oracle 66,76 % = plafond, le LLM pousse vers ce plafond. Reproductible, sans modèle lourd.)*
...
- **Critère A1 KS%** : `cargo xtask run-eval` sur le corpus complet → **n-gram (context-free) 35,49 % ≥ 25 % → ATTEINT** (oracle 66,76 % = plafond). Reproductible sans modèle lourd.
