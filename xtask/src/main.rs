...
fn main() -> ExitCode {
    ...
    match command.as_deref() {
        ...
        Some("check-contracts") => not_yet(
            "check-contracts",
            "Phase 1",
            "fluence-protocol defines no schemas yet",
        ),
...
fn print_usage() {
    ...
    eprintln!("  check-contracts       (Phase 1) verify generated schemas match fluence-protocol");
...
/// Runs the SPDX header check from the repository root (the parent of the
/// `xtask` crate, wherever cargo was invoked from).
